...
function WPPortfolio_convertShortcodeToPortfolio($atts)
{
	...
	if (is_numeric($single) && $single > 0)
	{
		...
		// Item found, so render it
			...
			return WPPortfolio_renderPortfolio($websiteDetails, false, false, false, false, false, false);
		}
	...
	return WPPortfolio_getAllPortfolioAsHTML($groups, false, $grouptemplate, $sitesperpage, $orderAscending, $orderType, false, false, $columns, $groupList);
}
 ...
 */
function WPPortfolio_getAllPortfolioAsHTML($groups = '', $template_website = false, $template_group = false, $sitesperpage = false, $orderAscending = true, $orderBy = 'normal', $count = false, $isWidgetTemplate = false, $columns = false, $groupList = false)
{
	if (!$groupList) {
		...
		$portfolioHTML = WPPortfolio_renderPortfolio( $websites, $template_website, $template_group, $paginghtml, $isWidgetTemplate, $columns, $message );
	}
	else {
		// Group list.
		global $wpdb;
...
function WPPortfolio_getRandomPortfolioSelectionAsHTML($groups = '', $count = 3, $template_website = false, $isWidgetTemplate = false, $columns = false)
{
	...
	$portfolioHTML = WPPortfolio_renderPortfolio($websites, $template_website, '', false, $isWidgetTemplate, $columns, $message);
 ...
 */
function WPPortfolio_renderPortfolio($websites, $template_website = false, $template_group = false, $paging_html = false, $isWidgetTemplate = false, $columns = false, $message = false)
{
	if (!$websites)
		return $message;
	...
	foreach ($websites as $websitedetails)
	{
		...
		if ($prev_group != $websitedetails->groupname)
		{
			...
			$renderedstr = WPPortfolio_replaceString(WPP_STR_GROUP_NAME, $websitedetails->groupname, $setting_template_group);
			$renderedstr = WPPortfolio_replaceString(WPP_STR_GROUP_DESCRIPTION, $websitedetails->groupdescription, $renderedstr);
